...
class Operadora extends CI_Controller
{
    ...
    public function index($id = false)
    {
        ...
        if (is_numeric($id)) {
            $data = $this->operadora_model->get($id);
        } else {
            $data = $this->operadora_model->get();
        }
    ...
    public function create()
    {
        ...
        $required_parameters = array('operadora', 'id_categoria', 'codigo', 'preco');

        $retval = $this->validate($required_parameters, $postdatas);
        if ($retval == 1) {
            $return = $this->operadora_model->create($postdatas);
            if ($return == true) {
        ...
        if (is_numeric($id)) {
            if (!isset($id)) {
                ...
                if ($retval == 1) {
                    ...
                    $return = $this->operadora_model->update($id, $postdatas);
                    if ($return == true) {
    ...
    public function delete($id = false)
    {
        if (is_numeric($id)) {
            if (!isset($id)) {
                ...
                $return = $this->operadora_model->delete($id);
                if ($return == true) {
