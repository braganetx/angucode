...
class Cores extends CI_Controller
{
    ...
    public function index($id = false)
    {
        ...
        if (is_numeric($id)) {
            $data = $this->cores_model->get($id);
        } else {
            $data = $this->cores_model->get();
        }
    ...
    public function create()
    {
        ...
        $required_parameters = array('cor', 'hex');

        $retval = $this->validate($required_parameters, $postdatas);
        if ($retval == 1) {
            $return = $this->cores_model->create($postdatas);
            if ($return == true) {
        ...
        if (is_numeric($id)) {
            if (!isset($id)) {
                ...
                if ($retval == 1) {
                    ...
                    $return = $this->cores_model->update($id, $postdatas);
                    if ($return == true) {
    ...
    public function delete($id = false)
    {
        if (is_numeric($id)) {
            if (!isset($id)) {
                ...
                $return = $this->cores_model->delete($id);
                if ($return == true) {
